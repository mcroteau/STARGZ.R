...
public class ExperienceManager {
    ...
    final String FOREACH  = "<stargzr:foreach";
    final String ENDEACH  = "</stargzr:foreach>";
    final String IFSPEC   = "<stargzr:if";
    final String ENDIF    = "</stargzr:if>";
    final String SETVAR   = "<stargzr:set";
    final String OPENSPEC = "<stargzr:if spec=\"${";
    ...
    int getEndEach(int openIdx, List<DataPartial> basePartials) throws StargzrException {
        ...
        for (int qxro = openIdx + 1; qxro < basePartials.size(); qxro++) {
            ...
            if(openEach > 3)throw new StargzrException("too many nested <stargzr:foreach>.");
            if(basicEntry.contains(this.ENDEACH) && endEach == openEach && endEach != 0){
                return qxro + 1;
            }
        }
        throw new StargzrException("missing end </stargzr:foreach>");
    }
