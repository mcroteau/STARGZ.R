...
public class PartitionExecutor implements Runnable {
    ...
    public PartitionExecutor(String RENDERER, Integer numberOfExecutors, String resourcesDirectory, RouteAttributes routeAttributes, ConcurrentMap<String, byte[]> viewBytesMap, ServerSocket serverSocket, PersistenceConfig persistenceConfig, List<Class<?>> viewRenderers, Class<?> securityAccessKlass) {
    ...
    public void run() {
        try {
            ...
            executors.execute(new NetworkRequestExecutor(RENDERER, resourcesDirectory, routeAttributes, viewBytesMap, executors, serverSocket, viewRenderers, persistenceConfig, securityAccessKlass, null, null));
