...
public class NetworkRequestExecutor implements Runnable {
    ...
    public NetworkRequestExecutor(String RENDERER, String resourcesDirectory, RouteAttributes routeAttributes, ConcurrentMap<String, byte[]> viewBytesMap, ExecutorService executorService, ServerSocket serverSocket, List<Class<?>> viewRenderers, PersistenceConfig persistenceConfig, Class<?> securityAccessKlass, FlashMessage storedFlashMessage, ViewCache storedViewCache) throws InterruptedException {
...
//                System.out.println("req=>" + networkRequest.getRequestPath() + "     /===> " + headerLineElement);
            ...
            RouteResult routeResult = routeEndpointNegotiator.performNetworkRequest(RENDERER, resourcesDirectory, viewCache, flashMessage, networkRequest, networkResponse, securityAttributes, securityManager, viewRenderers, viewBytesMap);
